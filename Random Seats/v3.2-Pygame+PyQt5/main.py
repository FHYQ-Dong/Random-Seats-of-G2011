...
def choose_seats_numbers():
    ...
    class Window(QWidget):
        ...
        def InitUI(self):
            ...
            # 标签
            def getQLabel(parent,text:str="Label",target:tuple=(0,0),size:tuple=(0,0)):
                Label=QLabel(text,parent)
                if size==(0,0):
                    Label.resize(Label.sizeHint())
                else:
                    Label.resize(size[0],size[1])
                Label.move(target[0],target[1])
                Label.show()
                return Label

            # 输入框
            def getQLineEdit(parent,defaultText:str="",target:tuple=(0,0)):
                LineEdit=QLineEdit(parent)
                LineEdit.setText(defaultText)
                LineEdit.move(target[0],target[1])
                LineEdit.show()
                return LineEdit

            # 按钮
            def getQPushButton(parent,function,text:str="Button",target:tuple=(0,0),size:tuple=(0,0)):
                PushButton=QPushButton(text,parent)
                if size==(0,0):
                    PushButton.resize(PushButton.sizeHint())
                else:
                    PushButton.resize(size[0],size[1])
                PushButton.clicked.connect(function)
                PushButton.move(target[0],target[1])
                PushButton.show()
                return PushButton
                

            # 窗口设置        
            self.setGeometry(300,300,210,100)
            self.setWindowTitle("test")
            self.setWindowIcon(QIcon("icon.ico"))

            # 行
            self.Lines_Label=getQLabel(self,"行数：",(10,10),(40,20))
            self.Lines_Edit=getQLineEdit(self,str(lines_number),(50,10))

            # 列
            self.Columns_Label=getQLabel(self,"列数：",(10,40),(40,20))
            self.Columns_Edit=getQLineEdit(self,str(columns_number),(50,40))

            # 确定按钮
            self.Confirm_Button=getQPushButton(self,self.Confirm_Button_func,"确定",(70,70))

            self.show()

        def Confirm_Button_func(self):
            global lines_number,columns_number,already_get_lines_columns_number
            if self.Lines_Edit.text().isdigit() and self.Columns_Edit.text().isdigit()\
                and not int(self.Lines_Edit.text())==0 and not int(self.Columns_Edit.text())==0:
                lines_number=int(self.Lines_Edit.text())
                columns_number=int(self.Columns_Edit.text())
    ...
    choose_seats_numbers_surface=Window()
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type==MOUSEBUTTONUP:
                ...
                if IN(mouse_pos,recent_students_name_rect.topleft,
                        recent_students_name_rect.bottomright):
                    show_info(",".join(students_name) if not students_name==[] else "空","查看")
                # 选择姓名文件
                if IN(mouse_pos,select_button_word_rect.topleft,
                    select_button_word_rect.bottomright):
                    name_file_name=select_file("All Files(*.*)")
                    if name_file_name=="":
                        continue
            ...
            for event in pygame.event.get():
                ...
                if event.type==MOUSEBUTTONUP:
                    ...
                    # 导出.rsd
                            ...
                            save_rsd_button_word_rect.bottomright):
                        saveas_file_name=ask_saveas_file_name("数据文件(.rsd)",".rsd")
                    ...
                    # 导出.txt
                            ...
                            save_txt_button_word_rect.bottomright):
                        saveas_file_name=ask_saveas_file_name("文本文件(.txt)",".txt")
